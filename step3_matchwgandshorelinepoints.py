...
# load coastal line
coastline = gpd.read_file(os.path.join(grandparent_folder, inputcoastlineloc,"nz-coastline-mean-high-water.shp"))
...
coords_tr = np.array(list(zip(lastestuniquepoints.geometry.x, lastestuniquepoints.geometry.y)))
...
for i, (dist_row, idx_row) in tqdm(
    ...
    ux1 = lastestuniquepoints.loc[i, "ux1"] if "ux1" in lastestuniquepoints.columns else np.nan
    uy1 = lastestuniquepoints.loc[i, "uy1"] if "uy1" in lastestuniquepoints.columns else np.nan
    Unique_ID = lastestuniquepoints.loc[i, "Unique_ID"] if "Unique_ID" in lastestuniquepoints.columns else pd.nan
...
transect_wp_lines = gpd.GeoDataFrame(rows_intersection, geometry="geometry", crs=gdf_WG.crs)
#del rows
transect_wp = (
    transect_wp_lines[
...
transect_wp.to_file(os.path.join(grandparent_folder,outputloc,transect_wp_gpkg), driver="GPKG")
...
for i,color in zip(unique_transect_wp_index_right, colors):
    ...
    plt.scatter(
        ...
        label=f'Wave points {i:.0f}: ({n_subset:.0f})', zorder=2
    )
...
ax2.set_title(f"{shorelinepointfilename} \n total: {len(lastestuniquepoints):.0f}; " 
...
fig2.savefig(os.path.join(grandparent_folder,outputloc,f'{shorelinepointfilename}_points_wavepoints_alignment.png'), 
