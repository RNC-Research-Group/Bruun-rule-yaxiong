...
exportdata = gpd.GeoDataFrame(columns=data.columns, geometry="geometry", crs=data.crs)
...
for _, row in tqdm(data.iterrows(), total=n_data, desc="processing transects"):
    ...
    exportdata = pd.concat(
        [exportdata, gpd.GeoDataFrame([row], crs=data.crs)], ignore_index=True
    )
