...
df = pd.DataFrame({"latitude": lat, "longitude": lon, "seapoint": seapoint})
...
gdf_wave_selected = gdf_wave[
    gdf_wave.within(gdf_combined.geometry.iloc[0])
...
gdf_combined.boundary.plot(
    ax=ax,
    color="green",
    linewidth=2,
    label=f"Original + Buffer={buffer_dist:.0f} m",
    zorder=1,
)
gdf_wave.plot(
    ax=ax, color="black", marker="+", label="All WG", markersize=30, alpha=0.5, zorder=2
)
gdf_wave_selected.plot(
    ax=ax,
    color="none",
    edgecolor="red",
    marker="o",
    label=f"WG selected ({len(gdf_wave_selected):.0f})",
    markersize=50,
    zorder=3,
)
...
# sys.exit()
# savedata
gdf_wave_selected.to_file(
    os.path.join(grandparent_folder, outputloc, outputfilenamegpkg), driver="GPKG"
)
fig.savefig(
    os.path.join(grandparent_folder, outputloc, outputfilenamejpg),
    dpi=300,
    bbox_inches="tight",
)
