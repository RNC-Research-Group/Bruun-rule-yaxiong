...
for f in matched_files:
    with rio.open(f) as src:
        ...
        for i, geom in tqdm(enumerate(lastestuniquepoints.geometry), total=len(lastestuniquepoints), desc="Processing points"):
...
with rio.open(bathy_path) as src:
    ...
    for geom in tqdm(lastestuniquepoints.geometry, desc="Sampling DEM values from bathy"):
        try:
            val = list(src.sample([(geom.x, geom.y)]))[0][0]
            peak_val = val
        except Exception:
...
# savedata
lastestuniquepoints.to_file(os.path.join(grandparent_folder,outputloc,outputfilename),driver='gpkg')
...
im=lastestuniquepoints.plot(column=var, ax=ax, legend=True, cmap="viridis",
    markersize=40, edgecolor="k", linewidth=0.3)
...
ax.set_title(f"{shorelinepointfilename}: \n buffer={buffer_dist} m \n dunepeak={mean_var:.1f}±{std_var:.1f} m", fontsize=10)
ax.set_xlabel("x (m)")
ax.set_ylabel("y (m)")
ax.set_aspect("equal")
plt.tight_layout()
...
fig.savefig(os.path.join(grandparent_folder,outputloc,outputfigname), dpi=300, bbox_inches='tight')  
