...
outputfile_gpkg=f"wavedatasum_{shorelinepointfilename}_{mindate.date()}_{maxdate.date()}.gpkg"
outputfile_shp=f"wavedatasum_{shorelinepointfilename}_{mindate.date()}_{maxdate.date()}.shp"
# Variables to process
variables = ["hs", "t01", "t02", "fp", "t0m1"]
g=9.81

# Read gauge file
WG_locations = os.path.join(grandparent_folder,inputloc, f"WGselected/WGselected_{shorelinepointfilename}.gpkg")
...
for i in range(n_points):
    ...
    df_point_sorted=df_point.sort_values(by="Hs", ascending=False).reset_index(drop=True)
    exceedance_prob = 12 / (24 * 365)  # 12 h out of one year
    rank_index = int(np.round(exceedance_prob * len(time)-1)) #0-based index in python
...
gdf_sum = gdf_WG[["seapoint", "geometry"]].merge(sum_results_all, on="seapoint", how="right")

# Convert to GeoDataFrame (inherits CRS from gdf_WG)
gdf_sum = gpd.GeoDataFrame(gdf_sum, geometry="geometry", crs=gdf_WG.crs)
gdf_sum.to_file(os.path.join(grandparent_folder,outputloc, outputfile_gpkg), driver="GPKG") 
gdf_sum.to_file(os.path.join(grandparent_folder,outputloc, outputfile_shp), driver="ESRI Shapefile") 
print("Computed H12h/y, T12h/y, and Fp12h/y for all seapoints.")

# Define which variables to plot (Hs and the period variables)
vars_to_plot = ["Hs_12h_y", "T01_12h_y", "T02_12h_y", "T0m1_12h_y", "Tp_12h_y","Tmean","CD"]

# Loop over variables and create one figure per variable
for var in vars_to_plot:
    fig, ax = plt.subplots(figsize=(8, 6))
    im=gdf_sum.plot(column=var, ax=ax, legend=True, cmap="jet",
        markersize=40, edgecolor="none", linewidth=0.3)
    ...
    if var == "Hs_12h_y" or var == "CD":
        ax.set_title(f"{shorelinepointfilename}: {mindate.date()}-{maxdate.date()}\n {var}={mean_var:.1f}±{std_var:.1f} m", fontsize=10)
    else:
        ax.set_title(f"{shorelinepointfilename}: {mindate.date()}-{maxdate.date()}\n {var}={mean_var:.1f}±{std_var:.1f} s", fontsize=10)
    ...
    fig_path = os.path.join(grandparent_folder,outputloc, f"{shorelinepointfilename}_{mindate.date()}-{maxdate.date()}_{var}.png")
